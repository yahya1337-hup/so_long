...
void ft_mlx_init(t_game **game_info)
{
	...
	free_mem((*game_info)->map_info.map);
        exit(1);
    }
}
...
{
     (*info)->mlx_wind = mlx_new_window((*info)->mlx_ptr, TILE_SIZE * (*info)->whidth, TILE_SIZE * (*info)->height, "so_long");
    if ((*info)->mlx_wind == NULL)
    {
	    mlx_destroy_display((*info)->mlx_ptr);
        ft_putstr_fd("the fuction mlx_init filed\n",2);
	    free_mem((*info)->map_info.map);
        exit(1);
    }
}
